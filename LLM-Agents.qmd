...
### Finally, Some Code:
The benchmark calls three live APIs and rewrites the result files, so it only runs when explicitly asked for. By default this document is built from the committed
results of the original run (36 queries), which keeps the analysis below reproducible without spending API quota.

To re-run the benchmark end to end -- wiping the old .JSONL/.CSV files and regenerating them via 'Agents_Multi_step.py' -- render with `RUN_BENCHMARK=1`.

Note that the original results were produced with Groq's `llama-3.3-70b-versatile`, which has since been decommissioned. A re-run therefore uses a different
model (set `GROQ_MODEL` in `.env`) and will not reproduce the exact numbers reported here.


```{python}
import os

# The benchmark calls three live APIs (Groq, Tavily, OpenWeather) and
# overwrites the committed result files, so it is opt-in rather than
# automatic. Set RUN_BENCHMARK=1 to re-run it; otherwise this document
# is built from the committed results of the original run.
RUN_BENCHMARK = os.getenv("RUN_BENCHMARK", "").strip().lower() in {"1", "true", "yes"}
...
def reset_logs():
    """Delete old log files so a fresh benchmark starts clean."""
    for f in ["agent_runs.jsonl", "baseline_runs.jsonl",
              "agent_runs.csv", "baseline_runs.csv"]:
        if os.path.exists(f):
            os.remove(f)
            print(f"Deleted: {f}")
...
if RUN_BENCHMARK:
    print(f"RUN_BENCHMARK set - running the full live benchmark.")
    print(f"Model: {os.getenv('GROQ_MODEL', 'openai/gpt-oss-120b')}")
    reset_logs()
    ...
    missing = [f for f in RESULT_FILES if not os.path.exists(f)]
    if missing:
        raise FileNotFoundError(
            "Committed result files are missing: " + ", ".join(missing)
            + ". Re-generate them with: RUN_BENCHMARK=1 quarto render LLM-Agents.qmd"
        )
    print("Using committed benchmark results.")
    print("Set RUN_BENCHMARK=1 to re-run the live benchmark instead.")
    print()
